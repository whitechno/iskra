...
object TestMain_03_CreateWrite {
  ...
  def run_02_write(spark: SparkSession): Unit = {
    ...
    write(sds = sds, isSinglePartitionDir = true, path  = DataLocation.base + "partitioner/single/")
    write(sds = sds, isSinglePartitionDir = false, path = DataLocation.base + "partitioner/multi/")
  }
